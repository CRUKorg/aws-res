...
class ClusterStack(IdeaBaseStack):
    ...
    * Cluster Settings
    ...
    def __init__(self, scope: constructs.Construct,
        ...
        self.vpc_gateway_endpoints: Optional[Dict[str,
                                                  VpcGatewayEndpoint]] = None
        self.vpc_interface_endpoints: Optional[Dict[str,
                                                    VpcInterfaceEndpoint]] = None
    ...
    def build_backups(self):
        ...
        backup_vault_removal_policy = self.context.config().get_string(
            'cluster.backups.backup_vault.removal_policy', default='RETAIN')
        ...
        if Utils.is_not_empty(backup_vault_kms_key_id):
            backup_vault_encryption_key = kms.Key.from_key_arn(
                self.stack, 'backup-vault-kms-key', self.get_kms_key_arn(key_id=backup_vault_kms_key_id))
        ...
        backup_plan.backup_selection.node.add_dependency(
            backup_s3_create_policy)
        if enable_restore:
            backup_plan.backup_selection.node.add_dependency(
                backup_restore_policy)
            backup_plan.backup_selection.node.add_dependency(
                backup_s3_restore_policy)
    ...
    def build_private_hosted_zone(self):
        ...
        self.private_hosted_zone = route53.PrivateHostedZone.from_lookup(
            scope=self.stack,
            id=f'{zone_name}-private-hosted-zone',
    ...
    def build_cluster_prefix_list(self):
        ...
        cluster_prefix_list_custom_resource.node.add_dependency(
            lambda_function)
    ...
    def build_cluster_endpoints(self):
        ...
        self.cluster_endpoints_lambda.node.add_dependency(
            cluster_endpoints_policy)
        self.cluster_endpoints_lambda.node.add_dependency(
            cluster_endpoints_role)
        ...
        external_alb_subnets = self.public_subnets(
            SubnetFilterKeys.LOAD_BALANCER_SUBNETS) if is_public is True else self.private_subnets(SubnetFilterKeys.LOAD_BALANCER_SUBNETS)
        ...
        if self.external_certificate is None:
            external_acm_certificate_arn = self.context.config().get_string(
                'cluster.load_balancers.external_alb.certificates.acm_certificate_arn', required=True)
        else:
        ...
        self.external_alb_https_listener = elbv2.CfnListener(
            ...
            'https-listener',
            port=443,
            ssl_policy=self.context.config().get_string('cluster.load_balancers.external_alb.ssl_policy',
        ...
        self.internal_alb_https_listener = elbv2.CfnListener(
            ...
            'https-listener',
            port=443,
            ssl_policy=self.context.config().get_string('cluster.load_balancers.internal_alb.ssl_policy',
                                                        default='ELBSecurityPolicy-TLS13-1-2-2021-06'),
        ...
        self.internal_alb_https_listener.node.add_dependency(
        ...
        if self.context.config().is_module_enabled(constants.MODULE_VIRTUAL_DESKTOP_CONTROLLER):
            ...
            dcv_broker_client_listener_default_actions = self.get_alb_listener_default_actions(
                dcv_broker_client_listener_arn)
            dcv_broker_client_communication_port = self.context.config().get_int(
                'virtual-desktop-controller.dcv_broker.client_communication_port', required=True)
            self.internal_alb_dcv_broker_client_listener = elbv2.CfnListener(
                ...
                ssl_policy=self.context.config().get_string('virtual-desktop-controller.dcv_broker.ssl_policy',
                                                            default='ELBSecurityPolicy-TLS13-1-2-2021-06'),
                load_balancer_arn=self.internal_alb.load_balancer_arn,
                protocol='HTTPS',
                certificates=[
            ...
            dcv_broker_agent_listener_default_actions = self.get_alb_listener_default_actions(
                dcv_broker_agent_listener_arn)
            dcv_broker_agent_communication_port = self.context.config().get_int(
                'virtual-desktop-controller.dcv_broker.agent_communication_port', required=True)
            ...
            dcv_broker_gateway_listener_default_actions = self.get_alb_listener_default_actions(
                dcv_broker_gateway_listener_arn)
            dcv_broker_gateway_communication_port = self.context.config().get_int(
                'virtual-desktop-controller.dcv_broker.gateway_communication_port', required=True)
    ...
    def build_cluster_settings(self):
        ...
        load_balancer_subnets = self.context.config().get_list(
        ...
        infrastructure_host_subnets = self.context.config().get_list(
        ...
        if not self.context.config().get_bool('cluster.network.use_existing_vpc', False):
            ...
            for eip in self.vpc.nat_gateway_ips:
                cluster_settings['network.nat_gateway_ips'].append(
                    f'{eip.ref}')
        ...
        if not self.context.config().get_bool('cluster.load_balancers.external_alb.certificates.provided', required=True):
            cluster_settings['load_balancers.external_alb.certificates.certificate_secret_arn'] = self.external_certificate.get_att_string(
                'certificate_secret_arn')
            cluster_settings['load_balancers.external_alb.certificates.private_key_secret_arn'] = self.external_certificate.get_att_string(
                'private_key_secret_arn')
            cluster_settings['load_balancers.external_alb.certificates.acm_certificate_arn'] = self.external_certificate.get_att_string(
                'acm_certificate_arn')
        else:
            cluster_settings['load_balancers.external_alb.certificates.provided'] = self.context.config(
            ).get_string('cluster.load_balancers.external_alb.certificates.provided', required=True)
            cluster_settings['load_balancers.external_alb.certificates.acm_certificate_arn'] = self.context.config(
            ).get_string('cluster.load_balancers.external_alb.certificates.acm_certificate_arn', required=True)

        cluster_settings['load_balancers.internal_alb.certificates.certificate_secret_arn'] = self.internal_certificate.get_att_string(
            'certificate_secret_arn')
        cluster_settings['load_balancers.internal_alb.certificates.private_key_secret_arn'] = self.internal_certificate.get_att_string(
            'private_key_secret_arn')
        cluster_settings['load_balancers.internal_alb.certificates.acm_certificate_arn'] = self.internal_certificate.get_att_string(
            'acm_certificate_arn')
        cluster_settings[
            'load_balancers.internal_alb.certificates.custom_dns_name'] = f'internal-alb.{self.private_hosted_zone.zone_name}'
